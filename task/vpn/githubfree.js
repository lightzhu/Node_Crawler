    ...
    return new Promise(function (reslove, reject) {
      ...
      axios
        ...
        .then((response) => {
          ...
          // 获取更新时间
          that.create_time = $('#readme .Box-body p')
            ...
            .split('\n')[0]
            .trim()
          console.log(that.create_time[0].trim())
          // 处理ssr资源
          // let $ssrTable = $('#readme .Box-body .snippet-clipboard-content').find('code')
          // let data = $ssrTable.next()
          // console.log(data)
          // let list = that.dealData(
          //   data
          //     .split('<br>')
          //     .map((item) => item.trim())
          //     .filter((item) => !item.startsWith('trojan:')),
          //   $ssrTable.text()
          // )
          // 处理v2ray资源
          let $v2rayTable = $('#readme .Box-body .snippet-clipboard-content').find('code')
          ...
          that.dealData(
            $v2rayTable
              ...
              .split('\n')
              .map((item) => item.trim()),
            $('#readme .Box-body #user-content-v2ray').text()
          )
          reslove(that.list)
        })
